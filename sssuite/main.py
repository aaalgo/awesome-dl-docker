...
if args.mode == "train":
    ...
    for epoch in range(0, args.num_epochs):
        ...
        for i in range(num_iters):
            ...
            for j in range(args.batch_size):
                ...
                id = id_list[index]
                input_image = cv2.resize(load_image(train_input_names[id]),(args.crop_width,args.crop_height))
                output_image = cv2.resize(load_image(train_output_names[id]),(args.crop_width,args.crop_height))
            ...
            #                                               target_height=args.crop_height, target_width=args.crop_width).eval(session=sess)            # ***** THIS CAUSES A MEMORY LEAK AS NEW TENSORS KEEP GETTING CREATED *****
        ...
        if epoch % args.validation_step == 0:
	        ...
	        input_image = np.expand_dims(np.float32(cv2.resize(load_image(val_input_names[ind]),(args.crop_height, args.crop_width))),axis=0)/255.0
                gt = cv2.resize(load_image(val_output_names[ind]),(args.crop_height, args.crop_width))             
    ...
    #resize_height = int(height / (float(width) / args.crop_width)) 	     
